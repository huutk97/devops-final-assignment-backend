pipeline {
    agent any
    environment{
        FULL_IMAGE = "390844768698.dkr.ecr.ap-southeast-1.amazonaws.com/devops-final-assignment-backend:latest"
        TASK_DEFINITION =""
        NEW_TASK_DEFINITION=""
        NEW_TASK_INFO=""
        NEW_REVISION=""
    ...
    stages {
        ...
        stage('Upload image to ECR') {
            steps {
                sh 'aws ecr get-login-password --region ap-southeast-1 | docker login --username AWS --password-stdin 390844768698.dkr.ecr.ap-southeast-1.amazonaws.com'
